(() => {
  ...
  const endScoreNode = document.getElementById("endScore");
  const endBestNode = document.getElementById("endBest");
  ...
  const AUDIO_VERSION = "20260501publish";
  const GROUND_OBSTACLE_SCALE = 0.8;
  const GROUND_Y = 602;
  const HERO_X = 218;
  ...
  const SPEED_SCORE_GROWTH_RATE = 0.932;
  const SPEED_RESPONSE = 3.8;
  const SCENE_SPEED_RESPONSE = 10;
  const MAX_PARTICLES = 40;
  const MAX_AIR_JUMPS = 1;
  const DOUBLE_JUMP_VELOCITY_FACTOR = 0.94;
  ...
  function syncGameOverUi() {
    const open = game.state === "gameOver";
    gameOverPanel.classList.toggle("open", open);
    gameOverPanel.setAttribute("aria-hidden", String(!open));
    if (!open) return;
    endScoreNode.textContent = Math.floor(game.score).toString();
    endBestNode.textContent = game.best.toString();
  }
  ...
  function getSpeedProgress() {
    return clamp((game.speed - BASE_SPEED) / (MAX_SPEED - BASE_SPEED), 0, 1);
  }

  function getDensityProgress() {
    return clamp(Math.max(getSpeedProgress(), game.score / 900), 0, 1);
  }
  ...
  function startJump() {
    const p = game.awawa;
    if (!p.grounded && p.airJumpsUsed >= MAX_AIR_JUMPS) return false;
    const airJump = !p.grounded;
    if (airJump) {
      p.airJumpsUsed += 1;
      p.vy = JUMP_VELOCITY * DOUBLE_JUMP_VELOCITY_FACTOR;
    } else {
      p.airJumpsUsed = 0;
      p.vy = JUMP_VELOCITY;
    }
    ...
    p.squash = airJump ? 0.92 : 0.88;
    spawnDust(p.x - 38, airJump ? p.y - 12 : GROUND_Y - 8, airJump ? 0.9 : 1.1);
    playAudio("jump");
    return true;
  }
  ...
  function spawnObstacle(sceneSpeed) {
    ...
    const densityProgress = getDensityProgress();
    const minSpawnSeconds = candidate.requiredLead / sceneSpeed;
    const randomCadence = rand(1.18, 2.34) + candidate.width / 520 - densityProgress * 1.22;
    const previousRight = getRightmostObstacleEdge();
    const desiredGap = Math.max(
      candidate.requiredLead + SAFE_RECOVERY_BUFFER + rand(64, 188) - densityProgress * 286,
      rand(332, 660) + candidate.width * rand(0.46, 0.96) - densityProgress * 410,
    );
  ...
  function pickFeasibleObstacle(sceneSpeed) {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      ...
      const height = template.height * scale * (template.flying ? 1 : GROUND_OBSTACLE_SCALE);
  ...
  function getAwawaFrame() {
    ...
    const runAnimSpeed = game.state === "playing"
      ? 8 + speedProgress * 5
      : 5;
    const frameIndex = Math.floor(game.time * clamp(runAnimSpeed, 8, 13)) % assets.awawa.run.length;
  ...
  function drawStart() {
    ...
    const hideInfoPanel = game.handheldLayout;
    const logoScale = desktopStart ? 0.7 : 1;
    const panelScale = desktopStart ? 0.72 : 1;
    ...
    const panelWidth = Math.min(544, Math.max(320, availableWidth * 0.82)) * panelScale * 0.7;
    const panelHeight = 152 * panelScale * 0.8;
    const panelX = centerX - panelWidth / 2;
    const bestFont = (panelWidth < 300 ? 26 : panelWidth < 350 ? 30 : 36) * Math.max(panelScale, 0.9);
    const subtitleFont = (panelWidth < 300 ? 20 : panelWidth < 350 ? 23 : 26) * Math.max(panelScale, 0.9);
    ...
    const textCenterY = panelY + panelHeight * 0.5;
    const textLineGap = panelHeight * 0.34;
    const bestTextY = textCenterY - textLineGap * 0.5;
    const subtitleTextY = textCenterY + textLineGap * 0.5;
    drawOverlay(0.16);
    drawCenteredImage(assets.logo, centerX, logoY, logoWidth);
    if (!hideInfoPanel) {
      drawPanel(panelX, panelY, panelWidth, panelHeight);
      centerText("最高分 " + game.best, centerX, bestTextY, bestFont, "#5b321d");
      centerText("帶 AWAWA 跳過灰白岩石", centerX, subtitleTextY, subtitleFont, "#7b4428");
    }
    const playYBase = hideInfoPanel ? (desktopStart ? 568 : 586) : desktopStart ? 594 : 612;
    const playY = playYBase + topShift * 0.82 - upwardShift;
  ...
  function resizeLayout() {
    ...
    const handheldLayout = coarsePointer || viewportWidth <= 760;
    const fillEpsilon = desktopFullBleed || shortDesktop ? 0.0035 : 0;
    const scaleBase = desktopFullBleed ? coverScale : viewportAspect < W / H || shortDesktop ? fillHeightScale : fitScale;
    const scale = scaleBase + fillEpsilon;
    ...
    const verticalAnchor = shortDesktop ? 0.5 : 0.5;
    const anchorTopPx = viewportHeight * verticalAnchor - scaledHeight * 0.5;
    const actualTopPx = shortDesktop ? Math.floor(viewportHeight - scaledHeight) : anchorTopPx;
    ...
    const settingsCardWidth = Math.max(220, Math.min(overlayCardWidth, visibleWorldWidth - worldSafePad * 2 - 12));
    const settingsCardCenter = clamp(
      visibleLeft + visibleWorldWidth * 0.5,
      visibleLeft + worldSafePad + settingsCardWidth * 0.5,
      visibleRight - worldSafePad - settingsCardWidth * 0.5,
    );
    game.layoutScale = clampedScale;
    game.layoutMode = desktopFullBleed ? "desktop" : ultraNarrow ? "ultraNarrow" : "default";
    game.mobileLayout = mobileOrNarrow;
    game.handheldLayout = handheldLayout;
    ...
    gameShell.style.setProperty("--settings-card-width", `${settingsCardWidth}px`);
    gameShell.style.setProperty("--settings-card-center", `${settingsCardCenter}px`);
    ...
    gameViewport.classList.toggle("handheld", handheldLayout);
    const shellCenterY = Math.round(actualTopPx + scaledHeight * 0.5);
    gameShell.style.left = `${Math.round(left)}px`;
    gameShell.style.top = `${shellCenterY}px`;
  }

  function syncCanvasCursor() {
    canvas.style.cursor = game.settingsOpen || game.state === "paused" || game.state === "gameOver" ? "default" : "pointer";
  }
  ...
  function unlockAudio() {
    ...
    if (game.audio.context.state === "suspended") {
      game.audio.context.resume().catch(() => {}).finally(() => {
        ensureBgmAudio();
        if (game.audio.bgmRequested || game.state === "playing") playBgm();
      });
      return;
    }
    ensureBgmAudio();
  ...
  function ensureBgmAudio() {
    ...
    const retryPlay = () => {
      if (!game.audio.bgmRequested || !game.audio.unlocked) return;
      audio.volume = game.audio.musicVolume;
      audio.play().catch(() => {});
    };
  ...
  ["contextmenu", "selectstart", "dragstart"].forEach((eventName) => {
    gameViewport.addEventListener(eventName, (event) => event.preventDefault());
  ...
  fullscreenButton.addEventListener("pointerup", async (event) => {
    event.preventDefault();
    event.stopPropagation();
    await toggleFullscreen();
  });
  fullscreenButton.addEventListener("touchend", async (event) => {
    event.preventDefault();
    event.stopPropagation();
    await toggleFullscreen();
  }, { passive: false });
  ...
  resumeButton.addEventListener("pointerup", (event) => {
    event.preventDefault();
    event.stopPropagation();
    continueFromSettings();
  });
  ...
  retryButton.addEventListener("pointerup", (event) => {
    event.preventDefault();
    event.stopPropagation();
    restart();
  });
  ...
  endHomeButton.addEventListener("click", (event) => {
    event.stopPropagation();
    returnToStart();
  });
  endHomeButton.addEventListener("pointerup", (event) => {
    event.preventDefault();
    event.stopPropagation();
    returnToStart();
  });
  endHomeButton.addEventListener("touchend", (event) => {
    event.preventDefault();
    event.stopPropagation();
    returnToStart();
  }, { passive: false });
